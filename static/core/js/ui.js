$(function(){
    ...
    ui.createWidget = function(name, option){
        var ele = document.createElement('div');
        if(name in classReverseMap){
            ele.className = classReverseMap[name];
            return ui(ele, option);
        }
        return false;
    };
    ...
    ui.WidgetTabBar = function(option){
        ...
        if(!option.selected){
            ...
            $e.find('.' + option.tabClassName + '[key="'+option.selected+'"]').addClass('selected');
        }
    }
        ...
        $(this.element).
            ...
            each(function(){
                ...
                if(k == key){
                    ...
                    that.view && that.view[k] && that.view[k].show();
                }else{
                    $(this).removeClass('selected');
                    that.view && that.view[k] && that.view[k].hide();
                }
            });
        this.selected = key;
    ...
    ui.WidgetToggle.prototype.toggle = function(v){
        if(v === true || v === false){
            this.val(v)
        }else{
    ...
    ui.WidgetSelect = function(option){
        ...
        this.hideOptions = function(e){
            var ofs = $(that.optionList).offset();
    ...
    ui.WidgetSelect.prototype.select = function(k){
        if(this.value == k){
            if(this.options[k] && this.options[k].name){
    ...
    ui.WidgetSelect.prototype.showOptions = function(){
        ...
        var w = 0;
        that.$.css('min-width', ($(that.optionList).outerWidth()));
    ...
    ui.WidgetDateRange = function(option){
        if(!option){
            return;
        }
        option = initOption({
            start : Math.floor(new Date().getTime() / 1000) - 86400 * 7,
        ...
        $body.append(rf);
        var calendarFrom = this.calendarFrom = ui.mkCalendar({
            value : option.start
        });
        var calendarTo   = this.calendarTo   = ui.mkCalendar({
            value : option.end
        });
        this.onchange = option.change;
        rf.dwRowCal.appendChild(calendarFrom);
        rf.dwRowCal.appendChild(calendarTo);
        this.updateDisplay();
        this._hideFrame = function(e){
            that.hideFrame();
        };
        $(rf).on('mousedown', function(){
            return false;
        });
        $e.click(function(){
            that.showFrame();
        });
        $(rf.dwRowCtrl.dwGroup.dwBtnOk).click(function(){
            that.time_start = calendarFrom.val();
            that.time_end   = calendarTo.val();
            that.updateDisplay();
            that.hideFrame();
            that.onchange && that.onchange.call(that, that.time_start, that.time_end);
        });
        $(rf.dwRowCtrl.dwGroup.dwBtnCancel).click(function(){
            that.hideFrame();
        });
    }
    ui.WidgetDateRange.prototype = new ui.Widget();
    ui.WidgetDateRange.prototype.showFrame = function(){
        var $off = this.$.offset();
        $(this.rangeFrame).css({
            left : $off.left,
            top  : $off.top + this.$.outerHeight() + 8
        }).show();
        $(window).on('mousedown', this._hideFrame);
        this.calendarFrom.val(this.time_start);
        this.calendarTo.val(this.time_end);
        return this;
    };
    ui.WidgetDateRange.prototype.hideFrame = function(){
        $(window).off('mousedown', this._hideFrame);
        $(this.rangeFrame).hide();
        return this;
    };
    ...
    ui.WidgetDateRange.prototype.setRange = function(start, end){
    ...
    ui.mkCalendar = function(option){
        option = initOption({

        }, option);
        var cal = _tpl.dwCalendar.clone();
        var date = new Date();
        if(option.value){
            date.setTime(option.value * 1000);
        }
        function change(){
            option.change && option.change.call(cal, Math.ceil(date.getTime() / 1000));
        }
        cal.val = function(v){
            if(v === undefined){
