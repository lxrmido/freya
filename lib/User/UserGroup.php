...
class UserGroup{
	...
	public static function get_group($group){
		$g = DB::assoc("SELECT * FROM `user_group` WHERE `id`=:id", ['id'=>$group]);
		if(empty($g)){
			return false;
		}
		return $g;
	...
	public static function user_list($group, $offset, $count){
		$count = DB::one("SELECT COUNT(`id`) FROM `user_basic` WHERE `group`=:group", ['group'=>$group]);
		$list  = DB::all("SELECT `id`,`username`,`email`,`group`,`regdate`,`lastlogin`,`logintimes`,`lastip`,`ban` FROM `user_basic` WHERE `group`=:group", ['group'=>$group]);
	 ...
	 */
	public static function has_parent($id, $parent){
		$r = self::get_group($id);
		if(!$r){
			return false;
		}
		...
		return self::has_parent($r['parent'], $parent);
	}
