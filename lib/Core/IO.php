...
class IO{
		...
		if(empty($_REQUEST[$item])){
			if(isset($_REQUEST[$item])){
				...
				if($val !== null){
					...
					global $FREYA_LANGS;
					# 获取翻译
					$item = empty($FREYA_LANGS[$item]) ? $item : $FREYA_LANGS[$item];
					self::e(-1, "'$item' 值不能为空");
				}
			}
			...
			# ID列表
			case 'ids':
				return self::filt_id_list($_REQUEST[$item]);
			# 原值返回
			default:
				return $_REQUEST[$item];
			}
 ...
 array('&amp;', '&quot;', '&lt;', '&gt;'), $str))));
	...
	public static function filt_script($str){
		$str = preg_replace('/<script/i', '<noscript', $str);
		$str = preg_replace('/\/script>/i', '/noscript>', $str);
		return $str;
	}
	...
	public static function filt_id_list($str){
		$ids_raw = explode(',', $str);
		$ids = [];
	...
	public static function argval($item, $val = null){
		global $FREYA_ARGS;
		if(empty($FREYA_ARGS[$item])){
			return $val;
		}else{
			return $FREYA_ARGS[$item];
		}
	}
	...
	public static function error($error){
        ...
        global $FREYA_JSONP;
        global $FREYA_CONSOLE;
		if($FREYA_DIE_ON_ERROR){
			if($FREYA_JSON_IO){
				$error['throw'] = json_encode(
					array(
						'code'    => $error['code'],
						'message' => $error['message'],
                        'method'  => $FREYA_METHOD['module'].'.'.$FREYA_METHOD['method'],
                        'debug'   => $FREYA_METHOD['debug']
					),
	...
	public static function genret($ret){
		...
		if($FREYA_DIE_ON_ERROR){
			if($FREYA_JSON_IO){
                ...
                if($FREYA_JSONP){
                    self::h('text/javascript');
                    die("{$FREYA_METHOD['jsonp_handle']}($json_str,'{$FREYA_METHOD['jsonp_key']}')");
                }else{
                    self::h('application/json');
                    die($json_str);
                }
